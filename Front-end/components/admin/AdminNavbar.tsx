...
import { fetchAdminNotifications, markAllNotificationsRead, markNotificationRead, type AppNotification } from '@/lib/notifications';
...
export function AdminNavbar({ userEmail, userName, summaryText, onMenuClick, onLogout }: AdminNavbarProps) {
  ...
  return (
    <header className="sticky top-0 content-center h-18 z-40 border-b border-orange-900/40 bg-linear-to-r from-[#1a0800] via-[#2d0f00] to-[#1a0800] shadow-[0_4px_24px_rgba(0,0,0,0.5)]">
      <div className="flex items-center justify-between w-full px-6 sm:px-8 lg:px-10 py-3">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-lg font-semibold text-white sm:text-xl">Admin Dashboard</h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
            ...
            <AnimatePresence>
              {isNotificationsOpen && (
                <motion.div
                  ...
                  className="absolute right-0 top-[calc(100%+0.5rem)] w-80 border border-orange-900/50 bg-[#120500] shadow-2xl shadow-black/60 z-50"
                >
                  <div className="border-b border-orange-900/40 px-5 py-3">
                    <p className="text-sm font-semibold text-white">Notifications</p>
                    <p className="mt-0.5 text-xs text-orange-400/70">System alerts</p>
                  </div>
                  ...
                  <div className="border-t border-orange-900/40 px-5 py-2 flex items-center justify-between">
                    <button
                      type="button"
                      onClick={() => { setIsNotificationsOpen(false); router.push('/admin/notifications'); }}
          ...
          <div className="relative">
            ...
            >
              <span className="flex h-8 w-8 items-center justify-center bg-linear-to-br from-amber-500 to-orange-400 text-xs font-bold text-black">
                {avatarLabel}
              </span>
              <span className="hidden sm:flex sm:flex-col">
                <span className="text-sm font-medium text-white">{displayName}</span>
                <span className="max-w-48 truncate text-xs text-orange-400/70">{userEmail ?? 'admin@rivercitycourier.com'}</span>
              </span>
            ...
            <AnimatePresence>
              {isProfileOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -6 }}
                  transition={{ duration: 0.15 }}
                  className="absolute right-0 top-[calc(100%+0.5rem)] w-60 border border-orange-900/50 bg-[#120500] shadow-2xl shadow-black/60 z-50"
                >
                  <div className="border-b border-orange-900/40 px-5 py-3">
                    <p className="text-sm font-semibold text-white">{displayName}</p>
                    <p className="mt-0.5 text-xs text-orange-400/70">Admin account</p>
                  </div>
                  <div className="divide-y divide-orange-900/20">
                    {[
                      ...
                      <button
                        key={item.label}
                        type="button"
                        onClick={() => { if (item.path) router.push(item.path); setIsProfileOpen(false); }}
                        className="flex w-full items-center px-5 py-3 text-left text-sm text-orange-100/80 transition hover:bg-orange-950/60 hover:text-white"
                      >
                        {item.label}
                      </button>
                    ))}
