...
import { fetchProfile, updateProfile, type AdminProfile } from '@/lib/admin-profile';
...
export function ProfileScreen({ userEmail, userName }: ProfileScreenProps) {
  ...
  const handleSidebarSelect = useCallback(
    (item: string) => {
      ...
      const paths: Record<string, string> = {
        ...
        Reviews: '/admin/reviews', Profile: '/admin/profile', Support: '/admin/support',
      };
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(249,115,22,0.34),transparent_22%),radial-gradient(circle_at_top_right,rgba(251,146,60,0.14),transparent_18%),linear-gradient(180deg,#2b1606_0%,#0f172a_52%,#020617_100%)] text-white">
      ...
      <AdminNavbar
        userEmail={userEmail}
        userName={userName}
        summaryText="View and manage your admin account details."
        ...
        {loading ? (
          ...
          <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="border border-orange-900/40 bg-slate-950/60 p-6">
                <p className="text-[0.65rem] uppercase tracking-[0.35em] text-orange-400/80">Quick Links</p>
                <h2 className="mt-1 text-xl font-semibold text-white">Admin Actions</h2>
                <div className="mt-5 space-y-2">
                  {['Change Password', 'Activity Log', 'Notification Preferences'].map((item) => (
                    <div key={item} className="border border-orange-900/30 bg-orange-950/20 px-4 py-3 text-sm text-orange-300/70 transition hover:border-orange-500/30 hover:bg-orange-950/40 hover:text-orange-200">
                      {item}
                    </div>
                  ))}
                </div>
