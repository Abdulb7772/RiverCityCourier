...
import { DriverCompletedOrders } from './DriverCompletedOrders';
...
export function DriverCompletedOrdersClient({ userEmail, userName }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(249,115,22,0.34),transparent_22%),radial-gradient(circle_at_top_right,rgba(251,146,60,0.14),transparent_18%),linear-gradient(180deg,#2b1606_0%,#0f172a_52%,#020617_100%)] text-white">
      ...
      <DriverNavbar
        userEmail={userEmail}
        userName={userName}
        summaryText="View all completed deliveries."
      ...
      <main className="relative z-10">
        <DriverCompletedOrders userEmail={userEmail} />
      </main>
    </div>
  );
}
